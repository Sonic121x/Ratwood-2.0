...
/datum/status_effect/buff/moonlightdance/on_remove()
	. = ..()
	to_chat(owner, span_warning("诺克的银辉离开了我的视野。"))
	REMOVE_TRAIT(owner, TRAIT_DARKVISION, TRAIT_STATUS_EFFECT(id))

/atom/movable/screen/alert/status_effect/buff/moonlightdance
	name = "月光之舞"
	desc = "诺克如石般冰凉的触碰落在我的心智之上，为我带来智慧。"
...
/datum/status_effect/buff/clash/proc/guard_disrupted()
	if(ishuman(owner))
		...
		owner.balloon_alert_to_viewers("<font color = '#bb2b2b'><b>格挡被击破!</b></font>")
...
/datum/status_effect/buff/clash/proc/guard_cancelled()
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
		owner.balloon_alert_to_viewers("<font color = '#bb2b2b'><b>格挡已取消！</b></font>")
		H.bad_guard(span_warning("我大意失守，专注溃散！"), cheesy = TRUE)
...
/datum/status_effect/buff/clash/on_remove()
	...
	if(expired_naturally)
		owner.balloon_alert_to_viewers("<font color = '#ffffff'>格挡已失效!</font>")
