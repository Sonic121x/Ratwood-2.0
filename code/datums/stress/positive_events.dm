...
/datum/stressevent/menthasmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_boldgreen("喉咙里泛起一阵清凉。"))

/datum/stressevent/blackberrysmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖泛起一阵酸甜。"))
	
/datum/stressevent/applesmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖感受到酸爽与清凉。"))

/datum/stressevent/chocolatesmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_purple("舌尖泛起一阵宜人的生涩与苦味。"))
	
/datum/stressevent/strawberrysmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖泛起一阵愉悦的酸甜。"))
	
/datum/stressevent/carrotsmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖上满是胡萝卜的味道。"))
	
/datum/stressevent/limesmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖泛起一阵甜美清爽的感觉。"))
	
/datum/stressevent/salviasmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖泛起一阵辛辣、泥土与苦味的快感。"))
	
/datum/stressevent/valerianasmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖泛起一阵苦辣与酸涩。"))
	
/datum/stressevent/zweed
	timer = 5 MINUTES
	stressadd = -2
	desc = list(span_blue("喉咙里泛起一阵宜人的苦涩，灼烧与刮擦感。尼古丁与橡树皮的味道在口中留下愉悦的余味。"))

/datum/stressevent/jacksberriessmoke
	timer = 1 MINUTES
	stressadd = -1
	desc = list(span_blue("舌尖泛起一阵微酸微甜的快感。"))

/datum/stressevent/abysssmoke
	timer = 1 MINUTES
	stressadd = 0
	desc = list(span_blue("舌尖泛起一阵微酸微甜……还有咸味？一阵不悦的寒意顺着脊椎爬下。你无法摆脱身后有人凝视的感觉……"))
