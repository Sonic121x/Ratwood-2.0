...
/datum/intent/unarmed/punch/rmb_ranged(atom/target, mob/user)
	...
	if(ismob(target))
		...
		user.visible_message(span_warning("[user]嘲弄着[M]！"), span_warning("我在嘲弄[M]！"), ignored_mobs = targetl)
		user.emote("taunt")
		if(M.client)
...
/datum/intent/unarmed/grab/rmb_ranged(atom/target, mob/user)
	...
	if(ismob(target))
		var/mob/M = target
		var/list/targetl = list(target)
		user.visible_message(span_green("[user]示意[M]靠近。"), span_green("我示意[M]靠近。"), ignored_mobs = targetl)
		if(M.client)
			if(M.can_see_cone(user))
				to_chat(M, span_green("[user]示意我靠近。"))
		else
			M.beckoned(user)
	return
