...
/obj/structure/lever/attackby(obj/item/I, mob/user, params)
	...
	if(user.used_intent.type == /datum/intent/chisel )
		...
		if(do_after(user, 10))
			var/levername
			levername = stripped_input(user, "你想在这个拉杆上刻什么名字？", "", "", MAX_NAME_LEN)
			if (levername)
				name = levername + "（拉杆）"
				desc = "一个刻着名字的拉杆"
			else
...
/obj/structure/pressure_plate/attackby(obj/item/I, mob/user, params)
	...
	if(user.used_intent.type == /datum/intent/chisel )
		...
		if(do_after(user, 10))
			var/platename
			platename = stripped_input(user, "你想在这个压力板上刻什么名字？", "", "", MAX_NAME_LEN)
			if (platename)
				name = platename + "（压力板）"
				desc = "一块刻着名字的板"
			else
...
/obj/structure/englauncher/attack_hand(mob/user)
	...
	if(user.used_intent.type == /datum/intent/chisel )
		...
		if(do_after(user, 10))
			var/launchername
			launchername = stripped_input(user, "你想在这个发射器上刻什么名字？?", "", "", MAX_NAME_LEN)
			if (launchername)
...
/obj/structure/floordoor/attackby(mob/user)
	...
	if(user.used_intent.type == /datum/intent/chisel )
		...
		if(do_after(user, 10))
			var/hatchname
			hatchname = stripped_input(user, "你想在这个活门上刻什么名字？", "", "", MAX_NAME_LEN)
			if (hatchname)
				name = hatchname + "（活门）"
				desc = "一个刻着名字的活门"
			else
