...
/obj/effect/proc_holder/spell/invoked/gnoll_sniff/cast(list/targets, mob/user)
	...
	if(is_valid_hunted(target) && target != user)
		...
		to_chat(user, span_notice("你捕捉到了 <a href='?src=[REF(user)];task=gnoll_view_tracked;'>[target.real_name]</a>.的气味。狩猎开始了！"))
...
/obj/effect/proc_holder/spell/invoked/gnoll_sniff/proc/select_new_target(mob/user)
	...
	to_chat(user, "<span class='notice'>你将感官集中在 [selected_target.real_name] 身上。</span> [selection != last_selection ? "(<a href='?src=[REF(user)];task=gnoll_view_tracked;'>查看</a>)" : ""]")
	give_tracking_directions(user)
