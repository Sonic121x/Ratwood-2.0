...
/datum/gnoll_prefs/proc/gnoll_process_link(mob/user, list/href_list)
	...
	switch(action)
		...
		if("headshot")
			...
			var/new_headshot_link = tgui_input_text(user, "输入头像链接（https，支持 gyazo、lensdump、imgbox、catbox）：", "头像", headshot_link,  encode = FALSE)
		...
		if("img_gallery")
			...
			var/new_galleryimg = tgui_input_text(user, "输入图片链接（https，支持 gyazo、lensdump、imgbox、catbox）：", "图库图片",  encode = FALSE)
			...
			if(!valid_headshot_link(user, new_galleryimg))
				to_chat(user, "<span class='notice'>图片链接无效。请使用受支持网站（gyazo、lensdump、imgbox、catbox）的图片直链。</span>")
				new_galleryimg = null
				gnoll_show_ui(user)
				return
		...
		if("ooc_extra_img")
			...
			to_chat(user, "<span class='notice'>图片和视频会限制宽度，高度不限。支持的网站：catbox、discord、gyazo、lensdump、imgbox。</span>")
