...
//	dat += "<a href='?_src_=prefs;preference=tab;tab=3' [current_tab == 3 ? "class='linkOn'" : ""]>Keybinds</a>"
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			dat += "<br>"
			dat += "<a href='?_src_=prefs;preference=parchment_skin'>羊皮纸：[get_parchment_skin_display_name()]</a>"
			dat += "</td>"
...
//							dat += "| Default: [default_keys.Join(", ")]"
	...
	if(istype(N))
		...
		if(SSticker.current_state <= GAME_STATE_PREGAME)
			...
			dat += " - <a href='?_src_=prefs;preference=observe'>旁观</a>"
			dat += "<br><a href='byond://?src=[REF(N)];villains=1'><b><font color='red'>反派</font></b></a>"
	else
	...
	dat += "<br><a href='?_src_=prefs;preference=close_prefs'><b>关闭</b></a>"
	dat += "</td>"
	dat += "</tr>"
	dat += "</table>"
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("headshot")
					...
					var/new_headshot_link = tgui_input_text(user, "输入头像链接（https，支持的主机：gyazo, lensdump, imgbox, catbox, imgbb, filegarden）：", "头像", headshot_link,  encode = FALSE)
				...
				if("rumour")
					...
					if(length(new_rumour) > 750)
						to_chat(user, span_warning("谣言不能超过 750 个字符。"))
						ShowChoices(user)
						return
					rumour = new_rumour
				...
				if("gossip")
					...
					if(length(new_gossip) > 750)
						to_chat(user, span_notice("贵族八卦不能超过 750 个字符。"))
						ShowChoices(user)
						return
				...
				if("img_gallery")
					...
					var/new_galleryimg = tgui_input_text(user, "输入图像链接（https，支持的主机：gyazo, lensdump, imgbox, catbox, imgbb, filegarden）：", "图库图像",  encode = FALSE)
					...
					if(!valid_headshot_link(user, new_galleryimg))
						to_chat(user, "<span class='notice'>无效的图像链接。请确保它是来自有效主机(gyazo, lensdump, imgbox, catbox, imgbb, filegarden)的直接链接。</span>")
						new_galleryimg = null
						ShowChoices(user)
						return
				...
				if("ooc_extra_img")
					...
					to_chat(user, "<span class='notice'>图像/视频的宽度受限但高度不限。合适的主机：catbox, discord, gyazo, lensdump, imgbox, imgbb, filegarden。</span>")
					to_chat(user, "<font color='#d6d6d6'>留一个空格以删除它。</font>")
					to_chat(user, "<font color='red'>滥用此功能将导致你被禁止。</font>")
...
/proc/valid_headshot_link(mob/user, value, silent = FALSE, list/valid_extensions = list("jpg", "png", "jpeg"))
	...
	if(find_index != 9)
		if(!silent)
			to_chat(usr, "<span class='warning'>链接必须托管在以下站点之一：Gyazo、Lensdump、Imgbox、Catbox、ImgBB、File Garden</span>")
		return FALSE
	return TRUE
