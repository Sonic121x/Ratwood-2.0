...
/obj/structure/soil/proc/try_handle_harvest(obj/item/attacking_item, mob/user, params)
	if(istype(attacking_item, /obj/item/rogueweapon/sickle))
		if(!plant || !produce_ready)
			to_chat(user, span_warning("没有可以收获的东西！"))
			return TRUE
		user_harvests(user)
...
/obj/structure/soil/proc/try_handle_tilling(obj/item/attacking_item, mob/user, params)
	if(istype(attacking_item, /obj/item/rogueweapon/hoe))
		...
		to_chat(user, span_notice("我开始翻耕土壤……"))
		playsound(src,'sound/items/dig_shovel.ogg', 100, TRUE)
		if(do_after(user, get_farming_do_time(user, work_time), target = src))
			to_chat(user, span_notice("我翻耕了土壤。"))
			playsound(src,'sound/items/dig_shovel.ogg', 100, TRUE)
			user_till_soil(user)
			update_icon()
		return TRUE
...
/obj/structure/soil/proc/try_handle_watering(obj/item/attacking_item, mob/user, params)
	...
	if(istype(attacking_item, /obj/item/reagent_containers))
		...
		if(target_water <= 0)
			to_chat(user, span_warning("土壤已经很湿了！"))
			return TRUE
		...
		if(total_units <= 0)
			to_chat(user, span_warning("\the [container]里没有水！"))
			return TRUE
		if(clean_water > 0)
	...
	if(water_amount > 0)
		...
		to_chat(user, span_notice("我给土壤浇了水。"))
...
/obj/structure/soil/proc/try_handle_fertilizing(obj/item/attacking_item, mob/user, params)
	...
	if(istype(attacking_item, /obj/item/ash))
		...
		to_chat(user, span_notice("你将肥料拌入土壤中……"))
		fertilize_soil()
		qdel(attacking_item)
		update_icon()
		return TRUE
	if(fertilize_amount > 0)
		if(nutrition >= MAX_PLANT_NUTRITION * 0.8)
			to_chat(user, span_warning("土壤已经施过肥了！"))
		else
			to_chat(user, span_notice("我给土壤施了肥。"))
...
/obj/structure/soil/proc/try_handle_deweed(obj/item/attacking_item, mob/user, params)
	...
	if(attacking_item == null)
		...
		if(do_after(user, get_farming_do_time(user, 3 SECONDS), target = src))
			...
			to_chat(user, span_notice("我拔除了杂草。"))
			deweed()
			update_icon()
		return TRUE
	if(istype(attacking_item, /obj/item/rogueweapon/hoe))
		apply_farming_fatigue(user, 10)
		to_chat(user, span_notice("我用[attacking_item]除掉了杂草。"))
		deweed()
		update_icon()
		return TRUE
	return FALSE
...
/obj/structure/soil/proc/try_handle_flatten(obj/item/attacking_item, mob/user, params)
	if(plant)
		return FALSE
	if(istype(attacking_item, /obj/item/rogueweapon/shovel))
		to_chat(user, span_notice("我开始用\the [attacking_item]平整土堆……"))
		playsound(src,'sound/items/dig_shovel.ogg', 100, TRUE)
		if(do_after(user, get_farming_do_time(user, 3 SECONDS), target = src))
			if(plant)
				return FALSE
			apply_farming_fatigue(user, 10)
			playsound(src,'sound/items/dig_shovel.ogg', 100, TRUE)
			to_chat(user, span_notice("我铲平了土堆。"))
			decay_soil()
		return TRUE
	return FALSE
...
/obj/structure/soil/attack_hand(mob/user)
	if(plant && produce_ready)
		to_chat(user, span_notice("我开始收获农产品……"))
		if(do_after(user, get_farming_do_time(user, 4 SECONDS), target = src))
			playsound(src,'sound/items/seed.ogg', 100, FALSE)
			user_harvests(user)
		return
	if(plant && plant_dead)
		...
		if(do_after(user, get_farming_do_time(user, 6 SECONDS), target = src))
			if(!plant || !plant_dead)
				return
			apply_farming_fatigue(user, 10)
			to_chat(user, span_notice("我清除了作物。"))
			playsound(src,'sound/items/seed.ogg', 100, FALSE)
			uproot()
		return
	. = ..()
...
/obj/structure/soil/examine(mob/user)
	...
	if(plant)
		...
		if(matured)
			...
			if(can_read_growth_timers && !produce_ready)
				...
				. += span_info("按当前生长速度，距离下次收获约需[DisplayTimeText(adjusted_produce)]。")
		else
			. += span_info("它尚未长成。")
			if(can_read_growth_timers)
				...
				. += span_info("按当前生长速度，预计还需[DisplayTimeText(adjusted_mature)]才能长成。")
		if(produce_ready)
			. += span_info("它可以收获了。")
	...
	if(!plant)
		...
		if(custom_turf)
			...
			if(tree)
				if(tree.dead)
					. += span_warning("这里有一株枯死的树苗。用铲子将它铲除，腾出地块。")
				else
					switch(tree.stage)
						if(TREESAP_STAGE_SAPLING)
							. += span_info("一株小树苗正在这里扎根。")
						if(TREESAP_STAGE_SHRUB)
							. += span_info("一株小灌木正在这里茁壮生长。")
					if(can_read_growth_timers && tree.stage <= TREESAP_STAGE_SHRUB)
						...
						. += span_info("按当前生长速度，预计还需[DisplayTimeText(adj)]进入下一生长阶段。")
			else if(bush)
				if(bush.dead)
					. += span_warning("这里有一株枯死的灌木幼苗。用铲子将它铲除，腾出地块。")
				else
					switch(bush.stage)
						if(BUSHSAP_STAGE_SAPLING)
							. += span_info("一株灌木幼苗正在这里扎根。")
						if(BUSHSAP_STAGE_BUDDING)
							. += span_info("一株灌木幼苗正在生长，根系仍扎在土壤中。")
	...
	if(fertilized_time > 0)
		. += span_good("土壤中混入了特殊肥料。")
	if(pollination_time > 0)
		. += span_good("这片地块已经完成授粉。")
	...
	if(can_read_growth_timers)
		...
		if(natural_bonuses["total"] > 0)
			...
			if(natural_bonuses["living_light"] > 0)
				bonus_parts += "生命之光（[round(natural_bonuses["living_light"] * 100)]%）"
			if(natural_bonuses["dendor_rune"] > 0)
				bonus_parts += "登多尔符文（[round(natural_bonuses["dendor_rune"] * 100)]%）"
