	...
	if (!hidden_job)
		...
		if (istype(H, /mob/living/carbon/human))
			if (obsfuscated_job) // WANDERER
				GLOB.actors_list["Wanderers"] += list("[H.mobid]" = "[H.real_name] 饰演[get_species_display_name(Hu.dna.species.name)]冒险者<BR>")
			else
				GLOB.actors_list[department] += list("[H.mobid]" = "[H.real_name] 饰演[get_species_display_name(Hu.dna.species.name)] [H.mind.assigned_role]<BR>")
		else
			if (obsfuscated_job)
...
/datum/job/Topic(href, list/href_list)
	if(href_list["explainjob"])
		...
		if(istype(src,/datum/job/roguetown/jester))
			LAZYCLEARLIST(dat)
			dat = list("<font color = '#d151ab'><center>来吧，来吧，都来看普赛顿长眠之处！<br>让赛利克斯掷下骰子，<br>引我们走向不期而至的终末！<br>啊哈哈哈！</center>")
			dat += "<center><b><font size = 4>力量: ???</b><br>"
			dat += "<b>意志: ???</b><br>"
			dat += "<b>体质: ???</b><br>"
			dat += "<b>感知: ???</b><br>"
			dat += "<b>智力: ???</b><br>"
			dat += "<b>幸运: ???</b><br></center></font>"
		var/height = 550
		if(sclass_count >= 10)
			height = 925
		var/datum/browser/popup = new(usr, "classhelp", "<div style='text-align: center'>[display_title || title]</div>", nwidth = 475, nheight = height)
