...
/datum/outfit/job/roguetown/confessor/pre_equip(mob/living/carbon/human/H, visualsOnly)
	...
	if(H.mind)
		var/stats = list("我脚步迅捷（+1 速度 +1 意志）", "我比他人更强壮（+1 力量 +1 感知）") // Both add up to +9
		var/stat_choice = input(H, "普赛顿将你塑造成了什么？", "这便是普赛顿将我塑成的模样。") as anything in stats
		switch(stat_choice)
			if("我脚步迅捷（+1 速度 +1 意志）")
			//does nothing. these stats are already given by default
			if("我比他人更强壮（+1 力量 +1 感知）")
				H.change_stat(STATKEY_STR, 1)
				H.change_stat(STATKEY_PER, 1)
				H.change_stat(STATKEY_WIL, -1)
				H.change_stat(STATKEY_SPD, -1)
		var/weapons = list("受祝普赛顿匕首", "普赛顿手锤", "普赛顿短剑")
