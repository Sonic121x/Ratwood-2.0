...
/datum/outfit/job/roguetown/adventurer/sfighter/pre_equip(mob/living/carbon/human/H)
	...
	H.set_blindness(0)
	if(H.mind)
		var/weapons = list("长剑","钉锤","钩镰枪","大连枷（需 12 力量）","战斧","短剑与铁盾","铁军刀与木盾")
...
/datum/outfit/job/roguetown/adventurer/duelist/pre_equip(mob/living/carbon/human/H)
	...
	H.set_blindness(0)
	if(H.mind)
		var/weapons = list("刺剑与招架匕首","军刀与圆盾","单刃刀与圆盾","匕首与招架匕首","双持短剑","重型匕首与徒手强化","双持软剑")
		var/weapon_choice = input(H, "选择你的武器。", "执兵而起") as anything in weapons
		switch(weapon_choice)
			if("刺剑与招架匕首")
				H.adjust_skillrank_up_to(/datum/skill/combat/swords, SKILL_LEVEL_EXPERT, TRUE)
...
/datum/outfit/job/roguetown/adventurer/barbarian/pre_equip(mob/living/carbon/human/H, visualsOnly)
	...
	H.set_blindness(0)
	if(H.mind)
		var/weapons = list("青铜拳刃","青铜剑","青铜斧","青铜钉锤","青铜矛","修习 - 猎鞭者","修习 - 软剑","修习 - 徒手")
		var/weapon_choice = input(H, "选择你的武器。", "执兵而起。") as anything in weapons
		switch(weapon_choice)
			if("青铜拳刃")
...
/datum/outfit/job/roguetown/adventurer/ironclad/pre_equip(mob/living/carbon/human/H, visualsOnly)
	...
	if(H.mind)
		...
		var/helmets = list(
			...
			"骑士头盔"		= /obj/item/clothing/head/roguetown/helmet/heavy/knight/iron,
			"阿米特盔"		= /obj/item/clothing/head/roguetown/helmet/heavy/knight/armet/iron,
			"长吻阿米特盔"		= /obj/item/clothing/head/roguetown/helmet/heavy/knight/armet/iron/snouted,
	...
	H.set_blindness(0)
	if(H.mind)
		var/weapons = list("行刑剑","战锤与盾","连枷与盾","大战连枷","卢塞恩锤","巨斧","防御就是一切")
...
/datum/outfit/job/roguetown/adventurer/mhunter/pre_equip(mob/living/carbon/human/H, visualsOnly)
	...
	if(H.mind)
		var/silver = list("银匕首","银短剑","银骑士剑","银刺剑","银长剑","银阔剑","银行刑剑","银钉锤","银战锤","银晨星锤","银大战连枷，最低13力量","银鞭","银软剑","银战斧","银长柄斧","银长矛","银四分杖")
...
/datum/outfit/job/adventurer/amazon/pre_equip(mob/living/carbon/human/H, visualsOnly)
	...
	switch(armor_choice)
		...
		if("皮甲式样")
			...
			var/weapons = list("钢指虎","斧","剑","鞭","软剑","矛","老娘赤手空拳就够了！！！")
			var/weapon_choice = input(H, "选择你的武器。", "执兵而起") as anything in weapons
			switch(weapon_choice)
				if ("钢指虎")
					H.adjust_skillrank_up_to(/datum/skill/combat/unarmed, SKILL_LEVEL_JOURNEYMAN, TRUE)
		...
		if("铆钉皮甲式样")
			...
			var/weapons = list("拳刃","刺剑","鞭","软剑","钩镰枪","老娘赤手空拳就够了！！！")
