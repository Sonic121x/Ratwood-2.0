...
/datum/outfit/job/roguetown/adventurer/thug/pre_equip(mob/living/carbon/human/H)
	...
	switch(classchoice)
		...
		if("打手")
			H.mind.cosmetic_class_title = "打手"
		...
		if("滑头")
			H.mind.cosmetic_class_title = "滑头"
		...
		if("大块头")
			H.mind.cosmetic_class_title = "大块头"
			...
			var/techniques = list("飞身踢 - 击退 + 额外伤害", "锁喉摔 - 体力伤害", "眩晕击 - 眩晕减益", "头槌 - 易伤减益") // cool wrestling moves
			var/technique_choice = input(H,"选择你的招式。", "摔他们！") as anything in techniques
		...
		if("码头汉")
			H.mind.cosmetic_class_title = "码头汉"
			to_chat(H, span_warning("你年轻时回应了阿比索尔的呼唤，只不过走的是不怎么体面的路子，\
