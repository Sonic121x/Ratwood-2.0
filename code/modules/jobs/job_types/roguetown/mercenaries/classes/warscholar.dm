/datum/advclass/mercenary/warscholar
	name = "Naledi 大司教"
	tutorial ="你是一名 Naledi 大司教，曾随隐修贤者研习法道，通晓各类奥术。你更擅长留在后阵，强化同伴并扰乱敌人。"
...
/datum/outfit/job/roguetown/mercenary/warscholar/pre_equip(mob/living/carbon/human/H)
	...
	to_chat(H, span_warning("你是一名 Naledi 大司教，曾随隐修贤者研习法道，通晓各类奥术。你更擅长留在后阵，强化同伴并扰乱敌人。"))
	if(H.age == AGE_OLD)
	...
	if(H.mind)
		detailcolor = input("选择一种颜色。", "Naledi 配色") as anything in naledicolors
...
/datum/advclass/mercenary/warscholar/pontifex
	name = "Naledi 教长"
	tutorial = "你是一名 Naledi 教长，接受过将控场法术与徒手搏杀结合的复合式训练。虽说你在纯魔法领域不算出众，但正面厮杀时却比大多数法师更为致命。你将冷静而娴熟的技艺凝为杀意，使其化作一柄奥术之刃。"
...
/datum/outfit/job/roguetown/mercenary/warscholar_pontifex/pre_equip(mob/living/carbon/human/H)
	...
	to_chat(H, span_warning("你是一名 Naledi 教长，接受过将控场法术与徒手搏杀结合的复合式训练。虽说你在纯魔法领域不算出众，但正面厮杀时却比大多数法师更为致命。你将冷静而娴熟的技艺凝为杀意，使其化作一柄奥术之刃。"))
	if(H.mind)
		detailcolor = input("选择一种颜色。", "Naledi 配色") as anything in naledicolors
		detailcolor = naledicolors[detailcolor]
...
/datum/advclass/mercenary/warscholar/vizier
	name = "Naledi 维齐尔"
	tutorial = "你是一名 Naledi 维齐尔。你对神迹与圣咒的研究将你引向了隐秘的秘法。尽管 Psydon 的信徒长期难以引动那位众父的神性，但借由圣者之力的拼合，或许也能抵达近似的境地。"
...
/datum/outfit/job/roguetown/mercenary/warscholar_vizier/pre_equip(mob/living/carbon/human/H)
	...
	to_chat(H, span_warning("你是一名 Naledi 维齐尔。你对神迹与圣咒的研究将你引向了隐秘的秘法。尽管 Psydon 的信徒长期难以引动那位众父的神性，但借由圣者之力的拼合，或许也能抵达近似的境地。"))
	if(H.age == AGE_OLD)
