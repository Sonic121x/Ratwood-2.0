...
/mob/living/carbon/human/examine(mob/user)
	...
	if(name in unknown_names)
		...
		if(migrant_type)
			...
			. = list(span_info("ø ------------ ø\n这是 <EM>[used_name]</EM>，一名流浪的[race_name][used_title]。"))
		else if(job)
		...
		if ((dna?.species?.id != "gnoll") && (valid_headshot_link(src, headshot_link, TRUE)) && (user.client?.prefs.chatheadshot))
			if(display_as_wanderer)
				display1 = span_info("ø ------------ ø\n[chat_headshot(headshot_link)]\n这是 <EM>[used_name]</EM>，一名流浪的[race_name]。")
			else if(display_as_lowlife)
				display1 = span_info("ø ------------ ø\n[chat_headshot(headshot_link)]\n这是 <EM>[used_name]</EM>，一名下等人的[race_name]。")
			else if(used_title)
				display1 = span_info("ø ------------ ø\n[chat_headshot(headshot_link)]\n这是 <EM>[used_name]</EM>，一名[race_name][used_title]。")
			else
				display1 = span_info("ø ------------ ø\n[chat_headshot(headshot_link)]\n这是 <EM>[used_name]</EM>，一名[race_name]。")
		else
			if(display_as_wanderer)
				display1 = span_info("ø ------------ ø\n这是 <EM>[used_name]</EM>，一名流浪的[race_name]。")
			else if(display_as_lowlife)
				display1 = span_info("ø ------------ ø\n这是 <EM>[used_name]</EM>，一名下等的[race_name]。")
			else if(used_title)
				display1 = span_info("ø ------------ ø\n这是 <EM>[used_name]</EM>，一名[race_name][used_title]。")
			else
				display1 = span_info("ø ------------ ø\n这是 <EM>[used_name]</EM>，一名[race_name]。")
			...
			switch (pronouns)
				if (HE_HIM, SHE_HER_M)
					if(user.has_stress_event(/datum/stressevent/uncanny))
						. += span_beautiful_masc("[m1]令人不安地英俊……有什么深深不对劲。")
					else
						. += span_beautiful_masc("[m1]令人魂牵梦萦的英俊。")
				if (SHE_HER, HE_HIM_F)
					if(user.has_stress_event(/datum/stressevent/uncanny))
						. += span_beautiful_fem("[m1]令人不安地美丽……有什么深深不对劲。")
					else
						. += span_beautiful_fem("[m1]令人魂牵梦萦的美丽。")
				if (THEY_THEM, THEY_THEM_F, IT_ITS)
					if(user.has_stress_event(/datum/stressevent/uncanny))
						. += span_beautiful_nb("[m1]令人不安地迷人……有什么深深不对劲。")
					else
						. += span_beautiful_nb("[m1]令人魂牵梦萦的迷人。")

		if (HAS_TRAIT(src, TRAIT_BEAUTIFUL_UNCANNY))
			switch (pronouns)
				if (HE_HIM, SHE_HER_M)
					if(user.has_stress_event(/datum/stressevent/beautiful))
						. += span_beautiful_masc("[m1]拥有[p_es()]一种超凡脱俗的英俊。")
					else
						. += span_beautiful_masc("[m2]的外表有种诡异的不对劲。")
				if (SHE_HER, HE_HIM_F)
					if(user.has_stress_event(/datum/stressevent/beautiful))
						. += span_beautiful_fem("[m1]拥有[p_es()]一种超凡脱俗的美丽。")
					else
						. += span_beautiful_fem("[m2]的外表有种诡异的不对劲。")
				if (THEY_THEM, THEY_THEM_F, IT_ITS)
					if(user.has_stress_event(/datum/stressevent/beautiful))
						. += span_beautiful_nb("[m1]拥有[p_es()]一种超凡脱俗的魅力。")
					else
						. += span_beautiful_nb("[m2]的外表有种诡异的不对劲。")
...
//				msg += "[m3] a blank, absent-minded stare and appears completely unresponsive to anything. [t_He] may snap out of it soon."
	...
	if((dna?.species?.id != "gnoll") && (!obscure_name || client?.prefs.masked_examine) && (flavortext || headshot_link || ooc_notes))
		. += "<a href='?src=[REF(src)];task=view_headshot;'>进一步观察</a>"
	...
	for(var/obj/item/bodypart/branded_bodypart as anything in bodyparts)
		...
		if(istype(branded_bodypart, /obj/item/bodypart/chest))
			...
			if(length(buttocks.branded_writing_on_buttocks) && get_location_accessible(src, BODY_ZONE_PRECISE_GROIN))
				. += span_info("[capitalize(m2)]的臀部被烙上了") + "[span_boldwarning(buttocks.branded_writing_on_buttocks)]。"
