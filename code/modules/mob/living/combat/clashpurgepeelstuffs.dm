/mob/living/carbon/human/proc/process_clash(mob/user, obj/item/IM, obj/item/IU)
	...
	if(!IU)	//The opponent is trying to rawdog us with their bare hands while we have Guard up. We get a free attack on their active hand.
		...
		if(H.apply_damage(force, IM.damtype, affecting, armor_block))
			visible_message(span_suicide("[src]用[IM]刺伤了[user]的双手！"))
			affecting.bodypart_attacked_by(used_intent.blade_class, force, crit_message = TRUE, weapon = IM)
		else
			visible_message(span_suicide("[src]用[IM]撞击了[user]的双手！"))
	...
	if(H.has_status_effect(/datum/status_effect/buff/clash))	//They also have Clash active. It'll trigger the special event.
		...
		visible_message(span_suicide("[src]用[IM]反击了[H]！"))
		...
		H.Slowdown(3)
		to_chat(src, span_notice("[H]露出了破绽！"))
		remove_status_effect(/datum/status_effect/buff/clash)
...
/mob/living/carbon/human/proc/clash(mob/user, obj/item/IM, obj/item/IU)
	...
	if((!instantloss && !instantwin) || (instantloss && instantwin))	//We are both using normal weapons OR we're both using memes. Either way, proceed as normal.
		visible_message(span_boldwarning("[src]与[HU]的兵刃激烈相撞！"))
		...
		if(prob(prob_opp))
			...
			success = TRUE
		if(!success)
			to_chat(src, span_warningbig("平手！对手的成功几率为……[prob_opp]%"))
			to_chat(HU, span_warningbig("平手！对手的成功几率为……[prob_us]%"))
...
/mob/living/carbon/human/proc/disarmed(obj/item/I)
	visible_message(span_suicide("[src]被缴械了！"), 
...
/mob/living/carbon/human/proc/try_guard()
	...
	if(!get_active_held_item())
		if(get_skill_level(/datum/skill/combat/unarmed) < SKILL_LEVEL_JOURNEYMAN)
			to_chat(src, span_warning("我的徒手格斗技艺不足，无法空手防守！"))
			return FALSE
	...
	if(IsImmobilized() || IsOffBalanced() || incapacitated(ignore_restraints = TRUE)) //Not while we can't move.
		balloon_alert(src, "<font color = '#ffffff'>无法行动时不能防守！</font>")
		return FALSE
	if(has_status_effect(/datum/status_effect/debuff/exposed))
		balloon_alert(src, "<font color = '#ffffff'>露出破绽时不能防守！</font>")
		return FALSE
...
/mob/living/carbon/human/proc/purge_bait()
	if(!cmode)
		if(bait_stacks > 0)
			bait_stacks = 0
			to_chat(src, span_info("我重新集中精神、站稳脚步。再被佯攻引诱时，我不会失去平衡了。"))
