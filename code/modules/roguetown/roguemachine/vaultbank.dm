...
/obj/structure/roguemachine/vaultbank/proc/draft_personal_loan(mob/living/carbon/human/user, list/params)
	if(!istype(user))
		return
	//if(GLOB.dayspassed > SStreasury.loan_max_issuance_day) 已移除个人贷款的发放截止日期判断。
		//say("第 [SStreasury.loan_max_issuance_day] 天之后不得再开具新贷款。") 已移除超过截止日的拒绝提示。
		//playsound(src, 'sound/misc/machineno.ogg', 100, FALSE, -1) 已移除该日期拒绝对应的提示音。
		//return 不再因游戏天数提前返回，继续验证金额、期限和资金。
	var/datum/fund/F = get_linked_fund()
	if(!F)
		to_chat(user, span_warning("[src] 毫无反应——它的金柜未绑定。请通知管理员。"))
		return
...
/obj/structure/roguemachine/vaultbank/proc/draft_indenture(mob/living/carbon/human/user, list/params)
	if(!istype(user))
		return
	//if(GLOB.dayspassed > SStreasury.loan_max_issuance_day) 已移除机构契约的发放截止日期判断。
		//say("第 [SStreasury.loan_max_issuance_day] 天之后不得再开具新契约。") 已移除超过截止日的拒绝提示。
		//playsound(src, 'sound/misc/machineno.ogg', 100, FALSE, -1) 已移除该日期拒绝对应的提示音。
		//return 不再因游戏天数提前返回，继续验证目标、金额和资金。
	var/datum/fund/F = get_linked_fund()
	if(!F)
		to_chat(user, span_warning("[src] 毫无反应——它的金柜未绑定。请通知管理员。"))
		return
