...
/obj/effect/proc_holder/spell/invoked/ventriloquism/cast(list/targets, mob/user = usr)
	if(isobj(targets[1]))
		...
		var/input_message = sanitize(input(usr, "要让 [target] 说什么？", src) as null|text)
...
/datum/status_effect/matthios_favor/on_apply()
	...
	if(meister_balance > 0)
		...
		to_chat(owner, span_warning("马西奥斯从你的神经锁里顺走了 [stolen] 枚玛门！"))
	else
		to_chat(owner, span_notice("马西奥斯伸手去摸你的神经锁，却发现里面空空如也。真是个穷酸的傻瓜！"))
	. = ..()
...
/atom/movable/screen/alert/status_effect/buff/matthios_favor
	name = "马西奥斯的恩泽"
	desc = "那位自由之神带着歪斜的笑意，从你的神经锁里顺走了钱币。"
	icon_state = "status"
...
/datum/status_effect/dendor_favor/on_apply()
	...
	to_chat(owner, span_warning("你突然被刺膝草的一刺所激颤，登多尔的力量让你短时间内能在崎岖地面上自由奔行！"))
	. = ..()
...
/obj/effect/proc_holder/spell/invoked/xylixlian_luck/cast(list/targets,mob/living/carbon/human/user = usr)
	...
	switch(result)
		...
		if(MATTHIOS)
			...
			to_chat(user, span_danger("马西奥斯 从你的神经锁里偷走了钱财，好给你上一课何为贪婪！"))
	return ..()
