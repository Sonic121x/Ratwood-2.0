...
/datum/sex_action/chastityplay/cage_grind_pussy_other/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]前挺[user.p_their()]腰，让[user.p_their()][get_chastity_device_name(user)]贴上[target]裸露的阴部。"))
...
/datum/sex_action/chastityplay/cage_grind_pussy_other/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]缓慢而刻意地用[user.p_their()]贞操笼磨蹭[target]的阴部，栅栏温热地压着，却毫不退让……"))
...
/datum/sex_action/chastityplay/cage_grind_pussy_other/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]缓缓退开，[user.p_their()][get_chastity_device_name(user)]的金属从[target]身上抽离，发出最后一声刮擦。"))
