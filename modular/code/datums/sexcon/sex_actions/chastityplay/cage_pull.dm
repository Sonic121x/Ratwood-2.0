...
/datum/sex_action/chastityplay/cage_pull/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(HAS_TRAIT(target, TRAIT_CHASTITY_SPIKED))
		...
		user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]猛地向外拽动[target]的尖刺[get_chastity_device_name(target)]，每一次拉扯都让内侧的尖刺刮过[target.p_their()]被困住的血肉……"))
	...
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]猛拽[target]的[get_chastity_device_name(target)]，金属咬入并拖磨着它所压迫的一切……"))
