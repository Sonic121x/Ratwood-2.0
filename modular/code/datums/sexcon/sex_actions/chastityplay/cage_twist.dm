...
/datum/sex_action/chastityplay/cage_twist/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(HAS_TRAIT(target, TRAIT_CHASTITY_SPIKED))
		...
		user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]像开瓶钻般扭动[target]的尖刺[get_chastity_device_name(target)]，让内侧尖刺缓慢而可怖地画出一圈……"))
	...
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]拧扭[target]固定着的[get_chastity_device_name(target)]，整具装置在被困住的肌肤上滚磨，金属声刺耳作响……"))
