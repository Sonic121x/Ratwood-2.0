...
/datum/sex_action/chastityplay/fondle_cage/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(HAS_TRAIT(target, TRAIT_CHASTITY_SPIKED))
		user.sexcon_action_message(user.sexcon.spanify_force("[user][user.sexcon.get_generic_force_adjective()]摩擦并按压[target]带刺的贞操装置，每一次抽动都让尖刺更深地陷进去..."))
		...
		return
	user.sexcon_action_message(user.sexcon.spanify_force("[user][user.sexcon.get_generic_force_adjective()]用掌心挤压并揉弄[target]的[get_chastity_device_name(target)]，刻意地摆弄着冰冷金属..."))
