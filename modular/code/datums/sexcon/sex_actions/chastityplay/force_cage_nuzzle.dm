...
/datum/sex_action/chastityplay/force_cage_nuzzle/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]按住[target]的头，强硬地把它引向[user.p_their()][get_chastity_device_name(user)]。"))

/datum/sex_action/chastityplay/force_cage_nuzzle/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]把[target]的脸按在[user.p_their()][get_chastity_device_name(user)]上，[target.p_their()]鼻子和脸颊都被压向金属……"))
...
/datum/sex_action/chastityplay/force_cage_nuzzle/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]松开手劲，让[target]把[target.p_their()]脸从[user.p_their()][get_chastity_device_name(user)]前抽回。"))
