...
/datum/sex_action/chastityplay/force_cage_rim/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]一把抓住[target]，毫不客气地把[target.p_their()]脸塞到[user.p_their()]贞操盾下方！"))

/datum/sex_action/chastityplay/force_cage_rim/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]把[target]的脸牢牢卡在[user.p_their()]后庭贞操盾下，借着这股压制操控[target.p_their()]舌头的每一次动作……"))
...
/datum/sex_action/chastityplay/force_cage_rim/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]把[target]猛地推开，贞操盾从[target.p_their()]脸上挪开时带出一声轻微的湿响。"))
