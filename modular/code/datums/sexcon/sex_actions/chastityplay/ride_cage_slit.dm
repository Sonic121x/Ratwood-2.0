...
/datum/sex_action/chastityplay/ride_cage_slit/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]跨坐到[target]身上，向前挪动身体，直到[user.p_their()]肉缝严丝合缝地贴上[target.p_their()][get_chastity_device_name(target)]。"))

/datum/sex_action/chastityplay/ride_cage_slit/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]让[user.p_their()]肉缝沿着[target]的[get_chastity_device_name(target)]一路磨蹭，每一次来回都让金属从敏感的皮肉上拖刮过去……"))
...
/datum/sex_action/chastityplay/ride_cage_slit/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]慢慢向后仰去，将[user.p_their()]肉缝从[target]的[get_chastity_device_name(target)]上抬开，分离的动作安静而刻意。"))
