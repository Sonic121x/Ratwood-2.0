...
/datum/sex_action/chastityplay/tailprod_cage/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]将[user.p_their()]尾巴低低扫过，缠上[target]的[get_chastity_device_name(target)]，感受着那装置的分量。"))

/datum/sex_action/chastityplay/tailprod_cage/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.sexcon_action_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()]让[user.p_their()]尾巴沿着[target]的[get_chastity_device_name(target)]缓缓缠绕摩挲，尾尖耐心而熟练地压进栅栏之间的缝隙……"))
...
/datum/sex_action/chastityplay/tailprod_cage/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]将[user.p_their()]尾巴从[target]的[get_chastity_device_name(target)]上松开，任其垂落下来。"))
