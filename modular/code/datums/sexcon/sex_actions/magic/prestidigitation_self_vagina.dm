/datum/sex_action/magic/masturbate_vagina_prestidigitation
	name = "用法师之手揉弄自己的阴蒂"
...
/datum/sex_action/magic/masturbate_vagina_prestidigitation/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user]召出奥术之手，伸向自己的阴部……"), vision_distance = (user.sexcon.do_subtle_action ? 1 : DEFAULT_MESSAGE_RANGE))
...
/datum/sex_action/magic/masturbate_vagina_prestidigitation/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	user.sexcon_action_message(user.sexcon.spanify_force("[user]引导悬空的魔法之手[user.sexcon.get_generic_force_adjective(is_stealth = do_subtle)]揉弄着自己的阴蒂……"), vision_distance = (do_subtle ? 1 : DEFAULT_MESSAGE_RANGE))
	if(!do_subtle)
