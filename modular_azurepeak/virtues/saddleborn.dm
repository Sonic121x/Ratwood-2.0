...
/obj/effect/proc_holder/spell/self/choose_riding_virtue_mount/cast(list/targets, mob/living/carbon/human/user = usr)
	...
	if(HAS_TRAIT(user, TRAIT_NOBLE) && istype(the_real_honse, /mob/living/simple_animal/hostile/retaliate/rogue/saiga))
		var/saiga_barding = list("无","衬垫马铠","链甲马铠")
		var/saiga_barding_choice = input(user, "你为坐骑购置了什么防护？", "鞍上所生") as anything in saiga_barding
		...
		var/fogbeast_barding = list("无","衬垫马铠","链甲马铠")
		var/fogbeast_barding_choice = input(user, "你为坐骑购置了什么防护？", "鞍上所生") as anything in fogbeast_barding
...
/obj/effect/proc_holder/spell/self/saddleborn/proc/check_mount(mob/living/carbon/human/user)
	...
	if(QDELETED(honse) || honse.stat == DEAD)
		to_chat(user, span_warning("内克拉已经带走它了……"))
		return FALSE

	if(honse.has_buckled_mobs())
		to_chat(user, span_warning("你的坐骑背上得先没人骑着才行！"))
		return FALSE
	...
	if(!isturf(user.loc) || !can_mount_travel(get_area(user), honse))
		if(istype(honse, /mob/living/simple_animal/hostile/retaliate/rogue/drider))
			to_chat(user, span_warning("我必须身处户外，或是在幽暗地域。"))
		else
			to_chat(user, span_warning("你需要身处户外！"))
		return FALSE
...
/obj/effect/proc_holder/spell/self/saddleborn/whistle/cast(list/targets, mob/living/carbon/human/user)
	...
	if(do_after(user, callback_time) && check_mount(user))
		...
		if(!QDELETED(honse) && honse_base_loc)
			...
			honse.visible_message(span_notice("[honse]在召唤被打断后，重新回到视野中。"))
		revert_cast(user)
		return FALSE
