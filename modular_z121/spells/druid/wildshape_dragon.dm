/proc/z121_is_dragon_wildshape_eligible(mob/living/carbon/human/H)
	...
	if(H.job != "Druid" && H.advjob != "Druid")
		return FALSE
	// Require the original Beast Form spell specifically, not just any wildshape subtype.
	if(!H.mind.has_spell(/obj/effect/proc_holder/spell/self/wildshape, TRUE))
		return FALSE
	return TRUE

// This is a ritual-only druid upgrade. It uses the normal wildshape pipeline so it stays
// compatible with mind transfer, devotion sharing, and the existing shapeshift cleanup.
/obj/effect/proc_holder/spell/self/wildshape/dragon
	...
	cooldown_min = 50
	invocations = list("Treefather, awaken the dragon within me!")
...
/obj/effect/proc_holder/spell/self/wildshape/dragon/cast(list/targets, mob/living/carbon/human/user = usr)
	...
	if(user.has_status_effect(/datum/status_effect/debuff/submissive))
		to_chat(user, span_warning("Your will is too broken to awaken the dragon soul."))
		revert_cast()
		return FALSE
	...
	if(!z121_is_dragon_wildshape_eligible(user))
		to_chat(user, span_warning("Only Dendor's druids who have awakened the dragon soul may use this form."))
		revert_cast()
		return FALSE

	// Do not call the parent wildshape spell here: it would open the normal beast radial
	// menu and queue a second transformation before the dragon form is applied.
	user.Stun(30)
	user.Knockdown(30)
...
/mob/living/carbon/human/species/wildshape/dendor_dragon/gain_inherent_skills()
	...
	if(src.mind)
		...
		// Tuned down from the hostile dragon so the form feels powerful without replacing bosses.
		src.STASTR = 18
		src.STACON = 17
		src.STAWIL = 15
		src.STAPER = 13
...
/obj/effect/proc_holder/spell/self/dragontalons
	name = "Dragon Talons"
	desc = "Extend or retract your talons."
	overlay_state = "claws"
...
/obj/effect/proc_holder/spell/self/dragontalons/cast(mob/user = usr)
	...
	if(istype(active, /obj/item/rogueweapon/dragon_talon) || istype(inactive, /obj/item/rogueweapon/dragon_talon))
		...
		if(istype(inactive, /obj/item/rogueweapon/dragon_talon) && inactive != active)
			user.dropItemToGround(inactive, TRUE)
		to_chat(user, span_notice("My talons retract."))
		extended = FALSE
	else
		l = new(user, 1)
		r = new(user, 2)
		...
		to_chat(user, span_notice("My talons extend."))
		extended = TRUE
