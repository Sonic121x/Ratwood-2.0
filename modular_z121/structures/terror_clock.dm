...
// before the clock is allowed to summon. "no buildings within a range of 8".
#define TERROR_CLOCK_CLEAR_RANGE 8
...
/obj/structure/terror_clock
	...
	// Reuse the original grandfather-clock striking texture, exactly as asked.
...
/obj/structure/terror_clock/obj_break(damage_flag)
	// Prefix the current state with "b" -> "bclock", the broken variant.
...
/obj/structure/terror_clock/attack_hand(mob/user)
	...
	// Commit: lock the clock, ring it once, and warn everyone nearby.
	summoning = TRUE
