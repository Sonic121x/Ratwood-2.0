...
/datum/particle_weather/hail
	...
	warning_message = span_greenannounce("高空中的寒意正在凝结，云层也在头顶聚集。")
	late_warning_message = span_greenannounce("坚硬的冰粒开始砸向地面。")
...
/datum/particle_weather/hail/weather_act(mob/living/L)
	...
	if(prob(50))
		...
		if(L.apply_damage(rand(5, 10), BRUTE, BODY_ZONE_HEAD, armor_block))
			if(prob(25))
				to_chat(L, span_danger("猛烈的冰雹正劈头盖脸地砸向我！"))
		else
			if(prob(25))
				to_chat(L, span_warning("冰块噼啪作响地弹在我的头部护具上。"))
