...
/datum/particle_weather/sand_gentle
	...
	warning_message = span_greenannounce("干燥的阵风掠过大地，将松散的尘土卷离地面。")
	late_warning_message = span_greenannounce("低沉的风声卷着细沙与尘土一同袭来。")
...
/datum/particle_weather/sand_storm
	...
	warning_message = span_greenannounce("凶猛的狂风席卷大地，在贴近地面的地方卷起厚重尘云。")
	late_warning_message = span_greenannounce("狂风尖啸着卷来，带着足以遮蔽视线的沙尘。")
	scale_vol_with_severity = TRUE
	weather_sounds = list(/datum/looping_sound/sandstorm)
	indoor_weather_sounds = list(/datum/looping_sound/wind)
	minSeverity = 40
