...
/obj/effect/proc_holder/spell/self/learnspell/cast(list/targets, mob/user = usr)
	...
	for(var/i = 1, i <= spell_choices.len, i++)
		...
		if(spell_item.zizo_spell > user_evil)
			continue
		if(spell_item.learn_requires_admin && !user?.client?.holder)
			continue
	...
	if(alert(user, "[item.desc]", "[item.name]", "学习", "取消") == "Cancel") //gives a preview of the spell's description to let people know what a spell does
		return
